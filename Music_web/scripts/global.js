...
function highlightPage() {
	...
	for(var i = 0; i < links.length; i++) {
		...
		if(window.location.href.indexOf(linkurl) != -1) {
			...
			var linktext = links[i].lastChild.nodeValue.toLowerCase(); //获取链接的文本信息
...
function prepareSlideshow() {
	...
	preview.setAttribute('alt', 'a glimpse fo what awaits you');
...
function prepareInternalnav() {
	...
	var articels = document.getElementsByTagName('article');
	if(articels.length == 0) {
		return false;
	}
	var navs = articels[0].getElementsByTagName('nav');
	if(navs.length == 0) {
		return false;
	}
	var nav = navs[0];
...
function displayAbbreviations() {
	if(!document.getElementsByTagName || !document.createElem || !document.createTextNode) {
		return false;
	}
